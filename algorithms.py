...
def bfs(board_to_solve, order='LRUD'):
    ...
    queue = [root]
    visited = {tuple(root.board.flatten())}
    counter = 0
    max_depth = 0
    ...
    while queue:
        ...
        for move in order:
            ...
            board_tuple = tuple(child.board.flatten())
...
def dfs(board_to_solve, order='RLUD', max_depth=MAX_DEPTH):
    start = time.perf_counter()
    root = Node(board_to_solve)
    ...
    stack = [root]
    visited = {tuple(root.board.flatten()): 0}
    counter = 0
    max_depth_stat = 0
...
def astr(board_to_solve, solved_board, heurisitc):
    ...
    while priority_queue:
        priority, tie_breaker, object = heapq.heappop(priority_queue)
        counter += 1
